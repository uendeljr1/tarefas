        ...
        {tasks.length > 0 ? (
          tasks.map(task => (
            <View key={task.id} style={styles.taskItem}>
              <TouchableOpacity 
                style={task.completed ? styles.checkCircleFilled : styles.checkCircle} 
          ...
          <Text style={styles.noTasksText}>Nenhuma tarefa disponível.</Text>
        )}
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  checkCircleFilled: {
    ...
    height: 24,
    borderRadius: 12,
    backgroundColor: '#4CD964', // Cor verde para tarefas concluídas
    marginRight: 12,
  },
  taskContent: {
    flex: 1,
